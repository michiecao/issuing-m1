...
// Mock data for Single-FA + Multi Currency account (one bucket, multiple currency accounts)
const singleMultiCurrencyData = {
  totalBalance: {
    amount: '$26,092.34',
    currency: 'USD',
  },
  buckets: [
    {
      ...
      accounts: [
        ...
        },
        {
          ...
          cards: [],
          incomingEarnings: [
            { month: 'Jan', day: '14', amount: '€673.45', description: 'From Stripe → Main • EUR' },
            { month: 'Jan', day: '15', amount: '€805.75', description: 'From Stripe → Main • EUR' },
          ],
        },
      ],
    },
...
const EmbeddedView = ({ brand = "" }) => {
  ...
  // Get feature flags from platform settings (regular variables)
  const outboundPayments = getVariable('outboundPayments');
  const outboundTransfers = getVariable('outboundTransfers');
  const financialAddresses = getVariable('financialAddresses');
  ...
  const financialAccountData = accountType === 'single'
    ? singleAccountData
    : accountType === 'singleMultiCurrency'
      ? singleMultiCurrencyData
      : multiAccountData;

  // Build currencies list from variables
  const currenciesObj = getVariable('currencies', {});
