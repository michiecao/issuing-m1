            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '135px 0 16px' }}>
              <div style={{ width: 658, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 32 }}>
                <p style={{ fontSize: 20, margin: 0, lineHeight: '28px', letterSpacing: '0.3px' }}>
                  <strong style={{ color: '#21252c', fontWeight: 700 }}>Your Stripe account comes with these by default.</strong>{' '}
                  <span style={{ color: '#99a5b8', fontWeight: 400 }}>No setup needed — they're ready to use.</span>
                </p>
                  ...
                  {[
                    {
                      ...
                      desc: 'A Stripe-hosted account to store, send, and receive funds.',
                      visual: (
                        <div style={{ background: '#f5f6f8', borderRadius: 6, padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: 6 }}>
                          <div style={{ fontSize: 11, color: '#8792a2' }}>Available balance</div>
                          <div style={{ fontSize: 20, fontWeight: 700, color: '#21252c', letterSpacing: '-0.3px' }}>$0.00</div>
                          <div style={{ display: 'flex', gap: 6, marginTop: 4 }}>
                            {['Add funds', 'Send', 'Transfer'].map(a => (
                              <div key={a} style={{ fontSize: 11, color: '#533afd', background: '#f0eeff', borderRadius: 4, padding: '3px 8px', fontWeight: 500 }}>{a}</div>
                            ))}
                          </div>
                        </div>
                      ),
                    ...
                    {
                      name: 'Business card',
                      desc: 'A card for your team to spend directly from your Financial Account.',
                      visual: (
                        <div style={{ background: 'linear-gradient(135deg, #635bff 0%, #8b5cf6 100%)', borderRadius: 6, padding: '14px 16px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', height: 72 }}>
                          <div style={{ width: 22, height: 16, borderRadius: 3, background: 'rgba(255,255,255,0.4)' }} />
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
                            <span style={{ fontSize: 10, color: 'rgba(255,255,255,0.8)', letterSpacing: 0.3 }}>business</span>
                            <span style={{ fontSize: 10, fontWeight: 700, color: 'rgba(255,255,255,0.7)', letterSpacing: 1 }}>MC</span>
                          </div>
                        </div>
                      ),
                    },
                  ].map((item) => (
                    <div key={item.name} style={{ flex: 1, background: '#fff', border: '1.5px solid #ebeef1', borderRadius: 8, padding: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        <div style={{ fontSize: 14, fontWeight: 600, color: '#353a44', lineHeight: '20px', letterSpacing: '-0.15px' }}>{item.name}</div>
                        <span style={{ fontSize: 11, fontWeight: 600, color: '#228403', background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: 100, padding: '2px 8px' }}>Included</span>
                      </div>
                      {item.visual}
                      <div style={{ fontSize: 12, color: '#596171', lineHeight: '16px' }}>{item.desc}</div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '135px 0 16px' }}>
              <div style={{ width: 658, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 32 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    <div style={{ display: 'flex', gap: 8 }}>
                      {moreRecsProducts.map((p) => {
                        const isChecked = p.name === 'Cards to power your product';
                        return (
                        <div key={p.name} style={{ width: 'calc(50% - 4px)', background: isChecked ? '#fff' : '#f5f6f8', border: isChecked ? '2px solid #675dff' : '1px solid transparent', borderRadius: 8, padding: 16, display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                          {isChecked ? (
                            <div style={{ width: 14, height: 14, borderRadius: 4, background: '#675dff', border: '1px solid #675dff', flexShrink: 0, marginTop: 2, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                              <svg width="8" height="6" viewBox="0 0 9 7" fill="none"><path d="M1 3.5l2.5 2.5 4.5-5" stroke="#fff" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /></svg>
                            </div>
    ...
    <Browser url="dashboard.stripe.com/issuing" fillHeight>
      <div style={{ overflow: 'hidden' }}>
        <div style={{ transform: 'scale(0.7)', transformOrigin: 'top left', width: `${100 / 0.7}%` }}>
          <DashboardView simplifiedNav hideHeroToggle issuingHeadline="Build a card program for your customers" issuingDescription="Issue virtual and physical cards via API, with programmable spend controls and instant issuance." />
        </div>
      </div>
    </Browser>
  </div>
